if Code.ensure_loaded?(Mssqlex) do
  defmodule MssqlEcto.Connection do
    ...
    def to_constraints(
      ...
      Logger.debug(error)
      to_constraints(odbc_code, message, constraint_violations)
    end

    def to_constraints(_code, _msg, constraints),
      do: [unique: constraints]

    def to_constraints(_code, _msg, constraints),
        do: [foreign_key: constraints]

    def to_constraints(%Mssqlex.Error{
          postgres: %{code: :exclusion_violation, constraint: constraint}
        }),
        do: [exclusion: constraint]

    def to_constraints(%Mssqlex.Error{postgres: %{code: :check_violation, constraint: constraint}}),
        do: [check: constraint]
    ...
    def execute(conn, %{ref: ref} = query, params, opts) do
      case Mssqlex.execute(conn, query, params, opts) do
        ...
        {:error, %Mssqlex.Error{postgres: %{code: :feature_not_supported}} = err} ->
          {:reset, err}
